...
def make_dataset(split=0, data_root=None, data_list=None, sub_list=None):    
    ...
    for l_idx in tqdm(range(len(list_read))):
        ...
        # 如果图像中不包含 base_class，则label_class = []; 否则，包含了base_class，将作为训练集
        label_class = new_label_class    

        if len(label_class) > 0:
            image_label_list.append(item)      # 如果当前图像中包含base类别，则将其加入，image_label_list
            for c in label_class:              # 如果图像包含新类别，再将其加入到对应类别的 列表中，可以根据类别选出
                if c in sub_list:
...
class SemData(Dataset):
    def __init__(self, split=3, shot=1, max_sp=5, data_root=None, data_list=None, transform=None, mode='train', use_coco=False, use_split_coco=False):
        ...
        # sub_lost 是训练集，base_class
        # sub_val_list 是测试集。novel_class
        if self.mode == 'train':
    ...
    def __getitem__(self, index):
        ...
        if self.history_mask_list[index] is None:
            history_mask_by_idx = torch.zeros(2, 41, 41).fill_(0.0)
        else:
            if random.random() > self.prob:
                history_mask_by_idx = self.history_mask_list[index]
            else:
                history_mask_by_idx = torch.zeros(2, 41, 41).fill_(0.0)

        if self.mode == 'train' and self.shot == 1:
            return image, label, s_x.squeeze(1), s_y.squeeze(1), subcls_list, history_mask_by_idx, index
        else:
            return image, label, s_x.squeeze(1), s_y.squeeze(1), subcls_list, history_mask_by_idx, index
