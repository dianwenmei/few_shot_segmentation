...
def Weighted_GAP(supp_feat, mask, init_memory):   # init_memory  20 * 256
    ...
    similairity = torch.matmul(supp_feat.squeeze(-1).squeeze(-1), init_memory.weight.T)   # B * 20
#     cos_simi = F.cosine_similarity(supp_feat.squeeze(-1).squeeze(-1), init_memory.weight).reshape(b, 20) #20 * 1
    #print(similairity.softmax(axis=-1).detach().cpu().numpy())
    aug_z = torch.matmul(similairity.softmax(axis=-1), init_memory.weight)   # b * 256
#     aug_z = torch.matmul(cos_simi, init_memory.weight)   # b * 256 
...
class Model(nn.Module):
    ...
    ######################################################
        self.boundry_layers = nn.Sequential(
            nn.Conv2d(reduce_dim*2, reduce_dim, kernel_size=3, padding=1, bias=False),
            nn.ReLU(inplace=True),
            nn.Dropout2d(p=0.5)
        )
        self.boundry_skip1 = nn.Sequential(
                nn.ReLU(inplace=True),
                nn.Conv2d(reduce_dim, reduce_dim, kernel_size=3, stride=1, padding=1, bias=False),
                nn.ReLU(inplace=True),
                nn.Conv2d(reduce_dim, reduce_dim, kernel_size=3, stride=1, padding=1, bias=False)
        )
        self.boundry_skip2 = nn.Sequential(
            nn.ReLU(inplace=True),
            nn.Conv2d(reduce_dim, reduce_dim, kernel_size=3, stride=1, padding=1, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(reduce_dim, reduce_dim, kernel_size=3, stride=1, padding=1, bias=False)
        )
        self.boundry_skip3 = nn.Sequential(
            nn.ReLU(inplace=True),
            nn.Conv2d(reduce_dim, reduce_dim, kernel_size=3, stride=1, padding=1, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(reduce_dim, reduce_dim, kernel_size=3, stride=1, padding=1, bias=False)
        )
        self..boundry_cls = nn.Sequential(
            nn.Conv2d(reduce_dim, reduce_dim, kernel_size=3, padding=1, bias=False),
            nn.ReLU(inplace=True),
            nn.Dropout2d(p=0.1),
            nn.Conv2d(reduce_dim, classes, kernel_size=1)
...
########################### Adaptive Superpixel Clustering ###########################
        ...
        for bs_ in range(bs):
            ...
            for shot_ in range(self.shot):
                ...
                supp_proto, aug_z = Weighted_GAP(supp_feat_.unsqueeze(0), supp_mask_.unsqueeze(0), self.init_memory)  # 1 x c x 1 x 1
