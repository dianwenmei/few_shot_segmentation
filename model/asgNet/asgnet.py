...
def Weighted_GAP(supp_feat, mask, global_class):
    supp_feat = supp_feat * mask
    feat_h, feat_w = supp_feat.shape[-2:][0], supp_feat.shape[-2:][1]
    area = F.avg_pool2d(mask, (supp_feat.size()[2], supp_feat.size()[3])) * feat_h * feat_w + 0.0005
    supp_feat = F.avg_pool2d(input=supp_feat, kernel_size=supp_feat.shape[-2:]) * feat_h * feat_w / area # 1 * c * 1 * 1

    global_represent = get_prototype(supp_feat, global_class) # 1 * c * 1 * 1

    return supp_feat# +0.1*global_represent.cuda(0)
...
def get_att_dis(target, behaviored):
    ...
    return attention_distribution / torch.sum(attention_distribution, 0)  # 标准�?
    ...
    def __init__(self, args):
        ...
        # Using ASPP as context module
            ...
            self.cls_aux = nn.Sequential(nn.Conv2d(reduce_dim, reduce_dim, kernel_size=3, padding=1, bias=False),
                                         ...
                                         nn.Dropout2d(p=0.1),
                                         nn.Conv2d(reduce_dim, classes, kernel_size=1))
        dic = np.load("/data/mdw/few_shot_segmentation/model/asgNet/dic_base_class_prototype.npy",
                      allow_pickle=True)
        dic_list = dic.tolist()
        for i in range(15):
            ...
            for shot_ in range(self.shot):
                with torch.no_grad():
                    ...
                    if (num_sp == 0) or (num_sp == 1):
                        supp_proto = Weighted_GAP(supp_feat_.unsqueeze(0), supp_mask_.unsqueeze(0), self.dic_tensor)  # 1 x c x 1 x 1
