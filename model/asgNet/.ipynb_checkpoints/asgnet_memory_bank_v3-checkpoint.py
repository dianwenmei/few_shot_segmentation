...
import numpy as np
import model.asgNet.resnet as models
from model.module.decoder import build_decoder
from model.module.ASPP import ASPP

# Masked Average Pooling
def Weighted_GAP(supp_feat, mask, init_memory, k_w, v_w):   # init_memory  20 * 256
    supp_feat = supp_feat * mask
    feat_h, feat_w = supp_feat.shape[-2:][0], supp_feat.shape[-2:][1]
    area = F.avg_pool2d(mask, (supp_feat.size()[2], supp_feat.size()[3])) * feat_h * feat_w + 0.0005
    supp_feat = F.avg_pool2d(input=supp_feat, kernel_size=supp_feat.shape[-2:]) * feat_h * feat_w / area # b * c * 1 * 1
    # b, c, _, _ = supp_feat.size()
    k = k_w(init_memory.weight)   # 20 * 256
    v = v_w(init_memory.weight)  
    similairity = torch.matmul(supp_feat.squeeze(-1).squeeze(-1), k.t())   # B * 20
    aug_z = torch.matmul(similairity.softmax(axis=-1), v)   # b * 256
    aug_z = aug_z.unsqueeze(-1).unsqueeze(-1)      # b * 256 * 1 * 1

    # supp_feat_aug = torch.cat([supp_feat, aug_z], dim=1)  # b * 2c * 1 * 1


    # global_represent = get_prototype(supp_feat, global_class) # 1 * c * 1 * 1

    return supp_feat, aug_z

# def get_prototype(a, dic_tensor):
#     """
#    a : 1 * 256 * 1 * 1
#    dic_tensor: 15 * 256 * 1 * 1
#    return 1 * 256 * 1 * 1
#    """
#     result = get_att_dis(a, dic_tensor)
#     result = torch.unsqueeze(result,0)
#
#     dic_tensor = torch.squeeze(dic_tensor,2)
#     dic_tensor = torch.squeeze(dic_tensor,2)
#
#     result_finnal=torch.matmul(result, dic_tensor.cpu()) #1 * 256
#     result_finnal = torch.unsqueeze(result_finnal,2)
#     result_finnal = torch.unsqueeze(result_finnal,3)
#
#     return result_finnal

# def get_att_dis(target, behaviored):
#     attention_distribution = []
#
#     for i in range(behaviored.size(0)):
#         attention_score = torch.cosine_similarity(target.view(1,-1), behaviored[i].view(1, -1))  # 计算每一个元素与给定元素的余弦相似度
...
class Model(nn.Module):
    def __init__(self, args):
        ...
        if self.pyramid:
            ...
            for bin in self.pyramid_bins:
                self.corr_conv.append(nn.Sequential(
                    nn.Conv2d(reduce_dim * 3 + 1, reduce_dim, kernel_size=1, padding=0, bias=False),
                    nn.ReLU(inplace=True),
                ))
        ...
        # Using ASPP as context module
            ...
            self.corr_conv = nn.Sequential(
                nn.Conv2d(reduce_dim * 3 + 1, reduce_dim, kernel_size=3, padding=1, bias=False),
                nn.ReLU(inplace=True),
                nn.Dropout2d(p=0.5)
            )
            ...
            self.cls_aux = nn.Sequential(nn.Conv2d(reduce_dim, reduce_dim, kernel_size=3, padding=1, bias=False),
                                         ...
                                         nn.Dropout2d(p=0.1),
                                         nn.Conv2d(reduce_dim, classes, kernel_size=1))
            # dic = np.load("dic_base_class_prototype.npy",
            #               allow_pickle=True)
            # dic_list = dic.tolist()
            # for i in range(15):
        ...
        self.k_w = nn.Linear(256,256,False)
        self.v_w = nn.Linear(256,256,False)
    ...
    def forward(self, x, s_x=torch.FloatTensor(1,1,3,473,473).cuda(), s_y=torch.FloatTensor(1,1,473,473).cuda(), s_seed=None, y=None):
        ...
        for i in range(self.shot):
            mask = (s_y[:, i, :, :] == 1).float().unsqueeze(1) # masl --> B * 1 * w * h
            with torch.no_grad():
                supp_feat_0 = self.layer0(s_x[:, i, :, :, :]) # s_x[:, i, :, :, :]  --> B * c * w * h
                supp_feat_1 = self.layer1(supp_feat_0)
                supp_feat_2 = self.layer2(supp_feat_1)
                supp_feat_3 = self.layer3(supp_feat_2)
                mask = F.interpolate(mask, size=(supp_feat_3.size(2), supp_feat_3.size(3)), mode='bilinear',
                                     align_corners=True)
                mask_list.append(mask)  # Get all the downsampled mask  shot x [bs x 1 x h x w]
...
########################### Adaptive Superpixel Clustering ###########################
        ...
        for bs_ in range(bs):
            ...
            for shot_ in range(self.shot):
                ...
                supp_proto, aug_z = Weighted_GAP(supp_feat_.unsqueeze(0), supp_mask_.unsqueeze(0), self.init_memory,self.k_w, self.v_w)  # 1 x c x 1 x 1
                supp_proto_map = supp_proto.expand(-1,-1,query_feat_.size()[1], query_feat_.size()[2])
                aug_z_map = aug_z.expand(-1,-1,query_feat_.size()[1], query_feat_.size()[2])
                    # sp_center_list.append(supp_proto.squeeze().unsqueeze(-1))                    # c x 1
                continue
            prob_map_list.append(F.cosine_similarity(query_feat_.unsqueeze(0), supp_proto,dim=1 ).unsqueeze(0)) #  1 * 1 x h x w
            guide_feat_list.append(supp_proto_map)  # b 1 * c * h * w
            aug_z_list.append(aug_z_map)            # b 1 * c * h * w
#
...
#             guide_feat_list.append(guide_feat.unsqueeze(0))
        ...
        guide_feat_list = torch.cat(guide_feat_list, dim=0)  # b * c * h * w
        aug_z_list = torch.cat(aug_z_list, dim=0)            # b * c * h * w


        guide_feat = torch.cat([query_feat, guide_feat_list, aug_z_list], dim=1)   # bs x 3c x h x w
    ...
    def _optimizer(self, args):
        if self.pyramid:
            optimizer = torch.optim.SGD(
                [
                    {'params': self.down_conv.parameters(), 'lr':args.base_lr},
                    {'params': self.corr_conv.parameters(), 'lr':args.base_lr},
                    {'params': self.alpha_conv.parameters(), 'lr':args.base_lr},
                    {'params': self.beta_conv.parameters(), 'lr':args.base_lr},
                    {'params': self.inner_cls.parameters(), 'lr':args.base_lr},
                    {'params': self.res1.parameters(), 'lr':args.base_lr},
                    {'params': self.res2.parameters(), 'lr':args.base_lr},
                    {'params': self.cls.parameters(), 'lr':args.base_lr},
                    {'params': self.init_memory.parameters(), 'lr':args.base_lr * 10},
                    {'params': self.k_w.parameters(), 'lr':args.base_lr * 10},
                    {'params': self.v_w.parameters(), 'lr':args.base_lr * 10}
            ...
            optimizer = torch.optim.SGD(
                [
                    ...
                    {'params': self.init_memory.parameters()}
                ],
