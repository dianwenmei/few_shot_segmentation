...
class ResNet(nn.Module):
    ...
    def forward(self, query_rgb,support_rgb,support_mask,history_mask):
        ...
        with torch.no_grad:
            query_rgb = self.layer0(query_rgb)
        ...
        with torch.no_grad:
            support_rgb = self.layer0(support_rgb)
    ...
    def _optimizer(self, args):
        optimizer = torch.optim.SGD(
            [
                ...
                {'params': self.layer55.parameters()},
                {'params': self.layer6_0.parameters()},
                {'params': self.layer6_1.parameters()},
                {'params': self.layer6_2.parameters()},
                {'params': self.layer6_3.parameters()},
                {'params': self.layer6_4.parameters()},
                {'params': self.layer7.parameters()},
                {'params': self.layer9.parameters()},
                {'params': self.residule1.parameters()},
                {'params': self.residule2.parameters()},
                {'params': self.residule3.parameters()},
            ],
