...
from util import dataset_test
from util import transform, config
from util.util import AverageMeter, intersectionAndUnionGPU
from util.util import visualize_image
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    global val_data
    val_data = dataset_test.SemData(split=args.split, shot=args.shot, max_sp=args.max_sp, data_root=args.data_root,
...
def validate(val_loader, model, criterion, args):
    ...
    total_time = 0
    for e in range(20):
        for i, (input, target, s_input, s_mask, s_init_seed, subcls, ori_label,index, sample_class) in enumerate(val_loader):
            ...
            origin = cv2.resize(origin, dsize=(473, 473), interpolation=cv2.INTER_LINEAR)
            mask = cv2.resize(mask, dsize=(473, 473), interpolation=cv2.INTER_LINEAR)
