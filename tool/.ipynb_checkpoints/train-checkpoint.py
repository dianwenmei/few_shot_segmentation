...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='../data/config/pascal/pascal_asgnet_split0_resnet50.yaml', help='config file')
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    # 加载模型==========================
    # Todo: 加载模型
    model = eval(args.arch).Model(args) # asgnet_memory_bank_v2
...
def train(train_loader, model, optimizer, epoch):
    ...
    train_data_generator = tqdm.tqdm(train_loader)
    vis_key = 0
    print('Warmup: {}'.format(args.warmup))
    for i, (input, target, boundry_target, s_input, s_mask, s_init_seed, subcls) in enumerate(train_data_generator):
...
#         loss = main_loss + args.aux_weight * aux_loss + boundry_loss
        ...
        if (i + 1) % args.print_freq == 0 and main_process():
            train_data_generator.set_description('Epoch: [{}/{}][{}/{}] '
...
def validate(val_loader, model, criterion):
    ...
    union_meter = AverageMeter()
    target_meter = AverageMeter()
    val_data_generator = tqdm.tqdm(val_loader)
    if args.use_coco:
        split_gap = 20
    else:
        split_gap = 5
    ...
    for e in range(10):
        for i, (input, target, s_input, s_mask, s_init_seed, subcls, ori_label) in enumerate(val_data_generator):
            ...
            if ((i + 1) % (test_num/100) == 0) and main_process():
                ...
                #                                               loss_meter=loss_meter,
                #                                               accuracy=accuracy))
                val_data_generator.set_description('Test: [{}/{}] '
                            'Data {data_time.val:.3f} ({data_time.avg:.3f}) '
                            'Batch {batch_time.val:.3f} ({batch_time.avg:.3f}) '
                            'Loss {loss_meter.val:.4f} ({loss_meter.avg:.4f}) '
                            'Accuracy {accuracy:.4f}.'.format(iter_num* args.batch_size_val, test_num,
